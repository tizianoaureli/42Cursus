...
namespace ft
{
	...
	template <class Iterator1, class Iterator2>
	bool operator==(const base_iterator<Iterator1>& x, const base_iterator<Iterator2>& y) { return x.base() == y.base(); }
	...
	template <class Iterator>
	base_iterator<Iterator> operator+ (typename base_iterator<Iterator>::difference_type n,const base_iterator<Iterator>& rev_it) { return base_iterator<Iterator>(rev_it.base() + n); }

	template <class Iterator1, class Iterator2> 
	typename base_iterator<Iterator1>::difference_type
	operator-(const base_iterator<Iterator1>& x, const base_iterator<Iterator2>& y) { return x.base() - y.base(); }
	...
	public:
		...
		template< class U >
		const_reverse_iterator& operator=( const const_reverse_iterator<U>& other ) { _current = other.getCurrent(); return *this; }
		template< class U >
		const_reverse_iterator& operator=( const reverse_iterator<U>& other ) { _current = other.getCurrent(); return *this; }
		const value_type& operator*() const { Iterator tmp = _current; return *--tmp; }
		const value_type* operator->() const { Iterator tmp = _current;	--tmp; return tmp.operator->();	}
		reference operator[](difference_type n) const { return *(*this + n); }
		const_reverse_iterator& operator++() { --_current; return  *this; }
		const_reverse_iterator& operator--() { ++_current; return *this; }
		const_reverse_iterator operator++( int ) { const_reverse_iterator tmp = *this; --_current; return  tmp; }
		const_reverse_iterator operator--( int ) { const_reverse_iterator tmp = *this; ++_current; return tmp; }
		const_reverse_iterator operator+( difference_type n ) const { return const_reverse_iterator(_current - (n)); }
		const_reverse_iterator operator-( difference_type n ) const { return const_reverse_iterator(_current + (n)); }
		const_reverse_iterator& operator+=( difference_type n ) { _current -= (n); return *this; }
		const_reverse_iterator& operator-=( difference_type n ) { _current += (n); return *this; }
	};
